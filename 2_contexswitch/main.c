/*欲使 task3 print out fibonacci sequence 輸出至第30個結果,紅燈閃鑠用於減慢輸出速度與作為一個flag*/
#include <stdint.h>
#include <stdio.h>
#include "reg.h"
#include "blink.h"
...
void task3(void){
	int i,j[]={0,1,1};
	printf("[fibonacci sequence]:(output to f30)\n\r");
	for(i=0;i<30;i++){
		j[2]=j[0]+j[1];
		j[0]=j[1];
		j[1]=j[2];
		printf("the %d th data: %d\r\n",(i+1),j[2]);
		if(i==29){
			i=-1;j[0]=0;j[1]=1;j[2]=1;
			printf("*********one more time**********\n\r ");
			blink_count(LED_RED,1);//delay for output(too fast may collapse)
		}
		blink_count(LED_RED,1);//delay for output(too fast may collapse)
	}
...
int main(void)
{
	...
	setup_systick(100*168e6 / 8 / 100); //10*100 ms
